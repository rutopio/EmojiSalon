...
function emojiToUnicode(thisEmoji) {
  ...
  components.forEach((ele, _) => {
    if (ele.length === 1) {
      // ZWJ or EMOJI MODIFIER FITZPATRICK
...
function getOverrideStyleString() {
  return customizedPaletteColors
    .map(
      (rgbaColorArray, idx) => `${originalPaletteIndex[idx]} ${rgbaColorArray}`
    )
    .filter(
      (_, idx) =>
        !areColorsEqual(
...
function updateEmojiAndURL() {
  ...
  window.location.hash = `${emojiToUnicode(thisEmoji)}-${encodeURIComponent(
    encodeURL(overrideColors)
  )}`;
...
async function updateEmoji(thisEmoji, keepPalette) {
  ...
  fetchEmojiData(thisEmoji)
    .then((data) => {
      pathArray = data.d;
      paletteArray = data.f
        .map((color) => (color === null ? "#000000" : color))
        .map((color) =>
          color.match(/^#([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])$/)
            ? color.replace(
...
function setColorPickers(glyphId, keepPalette) {
  ...
  originalPaletteColors = originalPaletteIndex.map(
  ...
  img.onload = async function () {
    ...
    if (mission == 1) {
      triggerDownload(
        ...
        `${emojiToUnicode(
          document.getElementById("customized-emoji").innerHTML
        )}-EmojiSalon.png`
      );
    } else if (mission == 2) {
      canvas.toBlob((blob) =>
...
function selectRandomColor() {
  ...
  customizedPaletteColors.forEach((_, idx) => {
    const hexRndColor = rgbaToHexColor([
      getRandomColor(),
      getRandomColor(),
      getRandomColor(),
      255,
...
function loadEmojiPicker() {
  ...
  const emojiPickerDesktopContainer = document.getElementById(
    "emoji-picker-desktop"
  );
  const emojiPickerMobileContainer = document.getElementById(
    "emoji-picker-mobile"
  );
  // Desktop Size
...
function changeDownloadButtonIcon() {
  Array.from(document.getElementsByClassName("download-image")).forEach(
    function (element) {
...
`
  ...
  Array.from(document.getElementsByClassName("copy-image")).forEach(function (
    element
  ) {
    while (element.firstChild) {
      element.removeChild(element.firstChild);
...
Array.from(document.getElementsByClassName("random-emoji-button")).forEach(
  function (element) {
...
Array.from(document.getElementsByClassName("random-color-button")).forEach(
  function (element) {
    element.addEventListener("click", function () {
...
Array.from(document.getElementsByClassName("reset-button")).forEach(function (
  element
) {
  element.addEventListener("click", function () {
...
Array.from(document.getElementsByClassName("download-image")).forEach(function (
  element
) {
...
Array.from(document.getElementsByClassName("share-button")).forEach(function (
  element
) {
  element.addEventListener("click", async () => {
    updateCanvas(4);
    ...
    if (navigator.share && window.innerWidth < 768) {
      ...
      Array.from(document.getElementsByClassName("copy-link")).forEach(
        function (element) {
          while (element.firstChild) {
            element.removeChild(element.firstChild);
          }
          element.insertAdjacentHTML(
            "afterbegin",
            `<svg class="btn-icon" viewBox="0 0 16 16"> <path d="M4.715 6.542 3.343 7.914a3 3 0 1 0 4.243 4.243l1.828-1.829A3 3 0 0 0 8.586 5.5L8 6.086a1.002 1.002 0 0 0-.154.199 2 2 0 0 1 .861 3.337L6.88 11.45a2 2 0 1 1-2.83-2.83l.793-.792a4.018 4.018 0 0 1-.128-1.287z" /><path d="M6.586 4.672A3 3 0 0 0 7.414 9.5l.775-.776a2 2 0 0 1-.896-3.346L9.12 3.55a2 2 0 1 1 2.83 2.83l-.793.792c.112.42.155.855.128 1.287l1.372-1.372a3 3 0 1 0-4.243-4.243L6.586 4.672z" />
             </svg>`
          );
        }
      );
      Array.from(document.getElementsByClassName("copy-image")).forEach(
        function (element) {
          while (element.firstChild) {
            element.removeChild(element.firstChild);
          }
          element.insertAdjacentHTML(
            "afterbegin",
            `<svg class="btn-icon" viewBox="0 0 16 16">  <path fill-rule="evenodd" d="M10 1.5a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 .5.5h3a.5.5 0 0 0 .5-.5v-1Zm-5 0A1.5 1.5 0 0 1 6.5 0h3A1.5 1.5 0 0 1 11 1.5v1A1.5 1.5 0 0 1 9.5 4h-3A1.5 1.5 0 0 1 5 2.5v-1Zm-2 0h1v1A2.5 2.5 0 0 0 6.5 5h3A2.5 2.5 0 0 0 12 2.5v-1h1a2 2 0 0 1 2 2V14a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V3.5a2 2 0 0 1 2-2Z" />
                    </svg>`
          );
        }
      );
      document.getElementById(
        "code-area-html"
      ).innerHTML = `&lt;span class="mod-emoji"&gt; ${
        document.getElementById("customized-emoji").innerHTML
      } &lt;/span&gt;`;
      try {
...
Array.from(document.getElementsByClassName("share-facebook")).forEach(function (
  element
) {
  element.addEventListener("click", function () {
...
Array.from(document.getElementsByClassName("share-twitter")).forEach(function (
  element
) {
  element.addEventListener("click", function () {
...
Array.from(document.getElementsByClassName("download-svg")).forEach(function (
  element
) {
  element.addEventListener("click", function () {
...
Array.from(document.getElementsByClassName("copy-image")).forEach(function (
  ...
  element.addEventListener("click", function () {
    if (navigator.canShare) {
      ...
      element.insertAdjacentHTML(
        "afterbegin",
        `<svg class="btn-icon" viewBox="0 0 16 16"><path d="M6.5 0A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3Zm3 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5h3Z" /><path d="M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1A2.5 2.5 0 0 1 9.5 5h-3A2.5 2.5 0 0 1 4 2.5v-1Zm6.854 7.354-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 0 1 .708-.708L7.5 10.793l2.646-2.647a.5.5 0 0 1 .708.708Z" /></svg>`
      );
    }
  });
});

Array.from(document.getElementsByClassName("share-linkedin")).forEach(function (
  element
) {
  element.addEventListener("click", function () {
    const lineShareURL = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(
      currentURL
    )}`;
...
Array.from(document.getElementsByClassName("copy-link")).forEach(function (
  element
) {
  element.addEventListener("click", function () {
    ...
    element.insertAdjacentHTML(
      "afterbegin",
      `<svg class="btn-icon" viewBox="0 0 16 16"><path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
            </svg>`
    );
  });
});
...
async function fetchEmojiData(thisEmoji) {
  if (emojiToUnicode(thisEmoji) in emojiPathsAndColors) {
    ...
    if (category == null) {
      console.log(
        `Unknown Category, try fetch https://raw.githubusercontent.com/rutopio/EmojiSalon/svgProcess/src/data/${emojiToUnicode(
          thisEmoji
        )}.json`
      );
      const response = await fetch(
        `https://raw.githubusercontent.com/rutopio/EmojiSalon/svgProcess/src/data/${emojiToUnicode(
          thisEmoji
        )}.json`
      );
      if (!response.ok) {
      ...
      console.log(
        `New Emoji Category: ${category}, fetch https://raw.githubusercontent.com/rutopio/EmojiSalon/svgProcess/src/data/${category}.json`
      );
      const response = await fetch(
        `https://raw.githubusercontent.com/rutopio/EmojiSalon/svgProcess/src/data/${category}.json`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
    ...
    if (getOverrideStyleString().length > 0) {
      getOverrideStyleString()
        ...
        .forEach((pair) => {
          ...
          customizedPaletteColors[originalPaletteIndex.indexOf(parseInt(key))] =
            value;
        });
      ...
      svgData.push(
        `<path fill="${
          customizedPaletteColors[
            originalPaletteColors.indexOf(paletteArrayLocal[index])
          ]
        }"  d='${d}' />`
      );
    });
